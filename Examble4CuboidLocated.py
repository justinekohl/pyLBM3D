...
while t <= tMax:
    ...
    # apply BC at z=0
    fNew = Ex.applyNeumannBoundaryConditions(fNew, fColl, rho, cs, cc, w, sigmaBCZMin, sigma, area, dx, 'z', 0)
    
    # apply BC at z=max
    fNew = Ex.applyNeumannBoundaryConditions(fNew, fColl, rho, cs, cc, w, sigmaBCZMax, sigma, area, dx,
                                             'z', maxZ - 1)
    
    # apply BC at y=0
    fNew = Ex.applyNeumannBoundaryConditions(fNew, fColl, rho,  cs, cc, w, sigmaBCYMin, sigma, area, dx,
                                                             'y', 0)
    
    # apply BC at y=max
    fNew = Ex.applyNeumannBoundaryConditions(fNew, fColl, rho, cs, cc, w, sigmaBCYMax, sigma, area, dx,
                                                             'y', maxY - 1)
    ...
    fNew = Ex.applyNeumannBoundaryConditionsAtEdge(fNew, fColl,  rho,  cs, cc,  w, sigmaBCEdgeYZ,
                                                   sigmaBCEdgeYZ, sigma, area, dx,  'y', 0, 'z', 0)

    # apply BC at edge y = min, z = max
    fNew = Ex.applyNeumannBoundaryConditionsAtEdge(fNew, fColl,  rho,  cs, cc,  w, sigmaBCEdgeYZ ,
                                                                   sigmaBCEdgeYZ, sigma, area, dx,  'y', 0, 'z', maxZ-1)

    # apply BC at edge y = max, z = min
    fNew = Ex.applyNeumannBoundaryConditionsAtEdge(fNew, fColl,  rho,  cs, cc,  w, sigmaBCEdgeYZ ,
                                                                   sigmaBCEdgeYZ, sigma, area, dx,  'y', maxY-1, 'z', 0)

    # apply BC at edge y = max, z = max
    fNew = Ex.applyNeumannBoundaryConditionsAtEdge(fNew, fColl,  rho,  cs, cc,  w, sigmaBCEdgeYZ ,
                                                                   sigmaBCEdgeYZ, sigma, area, dx, 'y', maxY-1, 'z', maxZ-1)
